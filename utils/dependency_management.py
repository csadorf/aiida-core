...
def _find_linenos_of_dependencies_in_setup_json(dependencies):
    """Determine the line numbers of dependencies specified in 'setup.json'."""
    linenos = defaultdict(list)

    with open(ROOT / 'setup.json') as setup_json_file:
        lines = list(setup_json_file)

    # Determine the lines that correspond to affected dependencies in setup.json.
    for dependency in dependencies:
        for lineno, line in enumerate(lines):
            if str(dependency) in line:
                linenos[dependency].append(lineno)
    return linenos
...
@click.option(
    ...
    'as part of a GitHub actions workflow.'
)
def check_requirements(extras, github_annotate):  # pylint disable: too-many-locals-too-many-branches
    ...
    if any(not_installed.values()):
        setup_json_linenos = _find_linenos_of_dependencies_in_setup_json(not_installed)
